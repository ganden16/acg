...
@section('title', 'Dashboard Edit Blog | PT. Adhi Cahaya Global')
...
@section('content')
<form id="edit-blog-form" method="POST" action="{{ route('dashboard.blog.update', $blog->slug) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
  <div class="space-y-12">
    <div class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3">
      <div>
        ...
        <p class="mt-1 text-sm/6 text-gray-600">Edit your indonesian blog</p>
      </div>
       ...
       <div class="sm:col-span-4">
            ...
            <div class="mt-2">
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    <input type="text" name="title_id" id="title_id" class="block min-w-0 grow py-2 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-base" placeholder="title id (required)" value="{{ old('title_id', $blog->translation_id->title ?? '') }}" required>
                </div>
                 @error('title_id')
        ...
        <div class="sm:col-span-4">
            ...
            <div class="mt-2">
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    <input type="text" name="subtitle_id" id="subtitle_id" class="block min-w-0 grow py-2 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-base" placeholder="subtitle id (optional)" value="{{ old('subtitle_id', $blog->translation_id->subtitle ?? '') }}">
                </div>
                 @error('subtitle_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="col-span-full">
            <label for="fileImage" class="block text-sm/6 font-medium text-gray-900">Image</label>
            <div class="mt-2 flex justify-center rounded-lg border-2 border-dashed border-gray-900 px-6 py-10">
                <div class="text-center">
                    ...
                    <p class="text-xs/5 text-gray-600 mt-2" id="file-name">
                        @if($blog->image)
                            Current Image: {{ basename($blog->image) }}
                        @endif
                    </p>
    ...
    <div class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3">
      <div>
        ...
        <p class="mt-1 text-sm/6 text-gray-600">Edit your english blog</p>
      </div>
      ...
      <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
        <div class="sm:col-span-4">
            ...
            <div class="mt-2">
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    <input type="text" name="title_en" id="title_en" class="block min-w-0 grow py-2 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-base" placeholder="title en (required)" value="{{ old('title_en', $blog->translation_en->title ?? '') }}" required>
                </div>
                 @error('title_en')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="sm:col-span-4">
            ...
            <div class="mt-2">
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                    <input type="text" name="subtitle_en" id="subtitle_en" class="block min-w-0 grow py-2 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-base" placeholder="subtitle en (optional)" value="{{ old('subtitle_en', $blog->translation_en->subtitle ?? '') }}">
                </div>
                 @error('subtitle_en')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
    ...
    <div class="mt-6 flex items-center justify-end gap-x-6">
        <a href="{{ route('dashboard.blog.index') }}" class="underline text-sm font-semibold text-gray-900">Cancel</a>
     ...
     if (fileInput) {
        fileInput.addEventListener('change', function () {
            ...
            if (files.length > 0) {
                ...
                @if($blog->image)
                    fileNameElement.textContent = 'Current Image: {{ basename($blog->image) }}';
                @else
